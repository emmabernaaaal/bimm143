...
> but we want to get rid of the “length” columns. We want it to match
> the metadata data.

There are 19808 genes in this dataset and 7 experiements (samples)..

> by putting the code between \`\` , when we “knit” it, it will appear
> as what the code actually calculates.

Looks like there is an extra ‘length’ column in our countData that we
need to remove.
...
A little bit more tidying work is required, because there are some genes
that have no data. Let’s remove them\! (they have zero counts in all
experiments)

Take the sums across the rows (`rowsum()`), and if it equals 0, exclude
those.
...
![](class_16_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
![](class_16_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->
...
Install packages from BioConductor (in the console).
`BiocManager::install()`.
...
Now that we have our geneIDs in the correct format we can do pathway
analysis
...
What is in our results? We can use `attributes()` function to see what
is returned here.
...
```
    ...
    ## Info: Working in directory /Users/emmalene_bernal/Desktop/BIMM 143/R_projects/bimm143_github/class_16

    ## Info: Writing image file hsa04110.pathview.png

> all the colored genes are the things involved in the pathway.

![hsa04110.pathview.png]() \> imports this figure into our report
